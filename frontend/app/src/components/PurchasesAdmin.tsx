...
const PurchasesAdmin = () => {
  ...
  const getPurchaseHistory = async (token: string) => {
    const res = await axios.get(ServerAddress+"/api/purchases/all", {
      headers: {
        ...
        <table className="bg-amber-100 shadow-md rounded-2xl w-full text-left  ">
          <thead className=" ">
            <tr>
              ...
              <th className=" border-l  p-4">Sweet</th>
          ...
          <tbody className="">
            {purchases.map((p:Sweet, i) => (
              <tr className=" rounded-2xl p-12" key={i}>
                <td className="p-3   border-t rounde-2xl ">{p.userId.name?? "-"}</td>
                <td className="p-3  border-l  border-t rounde-2xl ">{p.sweetName?? "-"}</td>
